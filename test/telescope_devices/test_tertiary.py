...
nonexistent ALPACA endpoint. ``TertiaryOCA`` implements the interface via ASA
AutoSlew vendor actions on the *telescope* ALPACA device
(``getcurrentnasmythport``, ``tertiarystatus``, ``selectnasmythport``) and
decomposes the ``tertiarystatus`` JSON into individually GET-able fields.
...
class TertiaryOCAActionsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_nasmythport_uses_getcurrentnasmythport_action(self):
        device = _make_tertiary(TertiaryOCA)
        device._connector.put.return_value = "2"  # AutoSlew replies with a string
        result = await device.nasmythport()
    ...
    async def test_nasmythport_unparsable_reply_raises_2002_normal(self):
        device = _make_tertiary(TertiaryOCA)
        device._connector.put.return_value = "garbage"
        with self.assertRaises(TreeValueError) as ctx:
            await device.nasmythport()
    ...
    async def test_decomposed_status_fields(self):
        device = _make_tertiary(TertiaryOCA)
        device._connector.put.return_value = json.dumps(JK15_STATUS)
        for method, expected in [('angle', JK15_STATUS['Angle']),
                                 ('moving', False),
                                 ('motoron', True),
                                 ('errorraised', False),
                                 ('portname', 'ADR10')]:
